...
const App = () => {
  ...
  const renderTable = (data, title) => (
    <div className={`rounded-xl border shadow-md p-4 mt-4 transform transition-all duration-500 ease-in-out ${
      showFullReceipt ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'
    }`} style={{ willChange: 'transform, opacity' }}>
      <h3 className="font-semibold text-sm sm:text-base text-blue-600 dark:text-blue-400 mb-2">{title}</h3>
      <div className="overflow-x-auto">
        <table className="w-full table-auto text-xs sm:text-sm">
          ...
          <tbody>
            {data.map((item, idx) => (
              <tr key={idx} className={`transition-all duration-300 hover:bg-opacity-10 ${idx % 2 === 0 ? '' : (darkMode ? 'bg-gray-800' : 'bg-gray-50')}`}>
                <td className="px-2 py-1 truncate max-w-[120px]">{item.name}</td>
                <td className="px-2 py-1 text-right font-medium">{parseFloat(item.value).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <div className={`min-h-screen transition-all duration-500 ${
      darkMode ? 'bg-gray-900 text-white' : 'bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 text-gray-800'
      ...
      <div className="container mx-auto px-4 py-6 md:py-12 relative z-10 max-w-md">
        ...
        {/* Input Card */}
        <div className={`rounded-2xl shadow-xl backdrop-blur-sm border overflow-hidden transition-all duration-500 transform hover:scale-102 ${
          darkMode 
            ? 'bg-gray-800/90 border-gray-700 shadow-gray-900/20' 
            : 'bg-white/90 border-gray-100 shadow-purple-200/50'
        }`}>
          {/* Top Gradient Strip */}
          <div className={`px-4 py-3 ${darkMode ? 'bg-gradient-to-r from-indigo-900 to-purple-900' : 'bg-gradient-to-r from-blue-600 to-indigo-600'} text-white shadow-lg`}>
          ...
          <div className="p-4 space-y-4">
            {[
              { id: 'hourlyRate', label: 'Тарифная ставка (₽/ч)', value: hourlyRate, setValue: setHourlyRate, placeholder: 'Например, 154.70' },
              { id: 'hoursWorked', label: 'Отработано часов', value: hoursWorked, setValue: setHoursWorked, placeholder: 'Например, 151' },
              { id: 'lossPremium', label: 'Премия за потери (₽)', value: lossPremium, setValue: setLossPremium, placeholder: 'Например, 424.29' },
            ].map((field) => (
              <div key={field.id} className="space-y-1">
                <label htmlFor={field.id} className="block text-xs font-medium opacity-80">
                  {field.label}
                </label>
                <input
                  id={field.id}
          ...
          {/* Results Summary */}
          {results && (
            <div className={`result-section p-4 border-t transition-all duration-500 ${
              ...
              <div className="grid grid-cols-1 gap-3">
                <div className={`p-3 rounded-lg border shadow-sm transition-all duration-300 ${
                  ...
                  <h3 className="font-semibold text-xs sm:text-sm text-blue-600 dark:text-blue-400 mb-2">С приработком:</h3>
                  <ul className="space-y-1 text-xs">
                    <li className="flex justify-between"><span>Грязными:</span><span>{results.with.gross} ₽</span></li>
                    <li className="flex justify-between"><span>НДФЛ (13%):</span><span>{results.with.tax} ₽</span></li>
                    <li className="flex justify-between"><span>На руки:</span><span className="text-green-600 dark:text-green-400">{results.with.net} ₽</span></li>
                  </ul>
                </div>
                <div className={`p-3 rounded-lg border shadow-sm transition-all duration-300 ${
                  darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'
                }`}>
                  <h3 className="font-semibold text-xs sm:text-sm text-blue-600 dark:text-blue-400 mb-2">Без приработока:</h3>
                  <ul className="space-y-1 text-xs">
                    <li className="flex justify-between"><span>Грязными:</span><span>{results.without.gross} ₽</span></li>
                    <li className="flex justify-between"><span>НДФЛ (13%):</span><span>{results.without.tax} ₽</span></li>
                    <li className="flex justify-between"><span>На руки:</span><span className="text-green-600 dark:text-green-400">{results.without.net} ₽</span></li>
                  </ul>
                </div>
              </div>
              ...
              {fullReceipt && (
                <div className="mt-4">
                  ...
                  >
                    {showFullReceipt ? 'Скрыть полный квиток' : 'Показать полный квиток'}
                  </button>
          ...
          <div className={`text-center text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            &copy; 2025 Официально. Все права защищены. Автор: Александров Александр
          </div>
        </div>
      </div>
    </div>
